...
class FrontendAssetsSubscriber implements EventSubscriberInterface
{
    ...
    public function registerFrontendAssets(RequestEvent $e): void
    {
        ...
        $templateName = $this->getTemplateName($request);

        if (str_starts_with((string) $templateName, 'fe_page_caribbean_blue') && $this->scopeMatcher->isFrontendRequest($request)) {
    ...
    private function getTemplateName(Request $request): string|null
    {
        ...
        $layoutModel = $pageModel->getRelated('layout');
